...
module.exports = {
  ...
  async store(req, res) {
    ...
    if (!modelo || !marca_id || !ano || !preco || !foto) {
      res.status(400).json({
        erro: "Enviar modelo, marca_id, ano, preco e foto da camiseta",
      });
      return;
    }

    try {
      const novo = await knex("camisetas").insert({
        modelo,
        marca_id,
        ano,
        preco,
...
//    console.log(dados[0]);
    ...
    if (dados[0].destaque) {
      try {
        await knex("camisetas").update({ destaque: 0 }).where({ id });
      ...
      try {
        await knex("camisetas").update({ destaque: 1 }).where({ id });
        res.status(200).json({ ok: 1 });
      } catch (error) {
        res
          .status(400)
  ...
  async destroy(req, res) {
    const id = req.params.id; // ou:  const { id } = req.params
    try {
      await knex("camisetas").del().where({ id });
      res.status(200).json({ ok: 1 });
    } catch (error) {
